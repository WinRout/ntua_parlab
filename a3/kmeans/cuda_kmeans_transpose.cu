                          ...
                          float *devdelta)
{
	...
	/* TODO: Maybe something is missing here... should all threads run this?  it was if(1)*/
    /* done */
    if (tid < numObjs) {
        int   index, i;
        float dist, min_dist;
        ...
        min_dist = euclid_dist_2_transpose(numCoords, numObjs, numClusters, deviceObjects, deviceClusters, tid, 0);
        /* done */

        for (i=1; i<numClusters; i++) {
            /* TODO: call dist = euclid_dist_2(...) with correct objectId/clusterId */
            dist = euclid_dist_2_transpose(numCoords, numObjs, numClusters, deviceObjects, deviceClusters, tid, i);
            /* done */
						...
						int blockSize)  
{
    ...
    double time_gpu = 0, time_cpu = 0, time_transfers = 0, timer;
    ...
    fprintf(fp, "%s,%d,%lf,%lf,%lf,%lf,%lf,%lf\n", "Transpose", blockSize, timing/loop, timer_min, timer_max, time_cpu, time_gpu, time_transfers);
	fclose(fp); 
